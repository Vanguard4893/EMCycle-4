...
namespace Cass {
    namespace SerialIO
    {
        ...
        bool CassSerial::OpenPort() {
            ...
            //QObject::connect(&rxTimer, &QTimer::timeout, this, &CassSerial::handleRxMessageTimeout);
            QObject::connect(&rxTimer, &QTimer::timeout, &eventLoop, &QEventLoop::quit);
            QObject::connect(this, &CassSerial::serialRxDataCountReached, &eventLoop, &QEventLoop::quit);
            ...
            if (serialPortInUse != nullptr) {
                if (serialPortInUse->isOpen()) {
                    if (!eventLoop.isRunning()) {
                        ...
                        eventLoop.exec();
                        //We get here either by timeout or the correct byte count.
